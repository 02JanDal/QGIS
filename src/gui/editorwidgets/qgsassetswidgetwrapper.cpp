...
#include "moc_qgsassetswidgetwrapper.cpp"
...
QgsAssetsWidgetWrapper::QgsAssetsWidgetWrapper( QgsVectorLayer *layer, int fieldIdx, QWidget *editor, QgsMessageBar *messageBar, QWidget *parent )
  : QgsEditorWidgetWrapper( layer, fieldIdx, editor, parent )
  , mMessageBar( messageBar )
{
}

QVariant QgsAssetsWidgetWrapper::value() const
{
  if ( mQgsWidget )
  {
...
void QgsAssetsWidgetWrapper::showIndeterminateState()
{
  if ( mLineEdit )
  {
...
bool QgsAssetsWidgetWrapper::valid() const
{
  return mLineEdit || mLabel || mQgsWidget;
}

void QgsAssetsWidgetWrapper::updateProperties( const QgsFeature &feature )
{
...
void QgsAssetsWidgetWrapper::setFeature( const QgsFeature &feature )
{
...
QWidget *QgsAssetsWidgetWrapper::createWidget( QWidget *parent )
{
  mForm = qobject_cast<QgsAttributeForm *>( parent );

  if ( mForm )
    connect( mForm, &QgsAttributeForm::widgetValueChanged, this, &QgsAssetsWidgetWrapper::widgetValueChanged );

  return new QgsExternalResourceWidget( parent );
}

void QgsAssetsWidgetWrapper::initWidget( QWidget *editor )
{
...
void QgsAssetsWidgetWrapper::updateValues( const QVariant &value, const QVariantList & )
{
  if ( mLineEdit )
  {
...
void QgsAssetsWidgetWrapper::setEnabled( bool enabled )
{
  if ( mLineEdit )
...
void QgsAssetsWidgetWrapper::widgetValueChanged( const QString &attribute, const QVariant &newValue, bool attributeChanged )
{
  Q_UNUSED( newValue );
  if ( attributeChanged )
  {
...
void QgsAssetsWidgetWrapper::updateConstraintWidgetStatus()
{
  if ( mLineEdit )
  {
...
void QgsAssetsWidgetWrapper::updateFileWidgetExpressionContext()
{
  if ( !mQgsWidget || !layer() )
    return;
