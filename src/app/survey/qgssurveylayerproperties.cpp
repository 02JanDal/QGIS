...
#include "qgssurveylayerproperties.h"
#include "moc_qgssurveylayerproperties.cpp"
...
QgsSurveyLayerProperties::QgsSurveyLayerProperties( QgsSurveyLayer *layer, QgsMapCanvas *canvas, QgsMessageBar *, QWidget *parent, Qt::WindowFlags flags )
  : QgsLayerPropertiesDialog( layer, canvas, QStringLiteral( "QgsSurveyLayerProperties" ), parent, flags )
  , mLayer( layer )
{
  setupUi( this );

  connect( this, &QDialog::accepted, this, &QgsSurveyLayerProperties::apply );
  connect( this, &QDialog::rejected, this, &QgsSurveyLayerProperties::rollback );
  connect( buttonBox->button( QDialogButtonBox::Apply ), &QAbstractButton::clicked, this, &QgsSurveyLayerProperties::apply );
  connect( buttonBox, &QDialogButtonBox::helpRequested, this, &QgsSurveyLayerProperties::showHelp );

  connect( mCrsSelector, &QgsProjectionSelectionWidget::crsChanged, this, &QgsSurveyLayerProperties::crsChanged );
  ...
  if ( !settings.contains( QStringLiteral( "/Windows/SurveyLayerProperties/tab" ) ) )
  {
    settings.setValue( QStringLiteral( "Windows/SurveyLayerProperties/tab" ), mOptStackedWidget->indexOf( mOptsPage_Information ) );
  }

  mBtnStyle = new QPushButton( tr( "Style" ) );
  QMenu *menuStyle = new QMenu( this );
  menuStyle->addAction( tr( "Load Style…" ), this, &QgsSurveyLayerProperties::loadStyleFromFile );
  menuStyle->addAction( tr( "Save Style…" ), this, &QgsSurveyLayerProperties::saveStyleToFile );
  menuStyle->addSeparator();
  menuStyle->addAction( tr( "Save as Default" ), this, &QgsSurveyLayerProperties::saveStyleAsDefault );
  menuStyle->addAction( tr( "Restore Default" ), this, &QgsSurveyLayerProperties::loadDefaultStyle );
  mBtnStyle->setMenu( menuStyle );
  connect( menuStyle, &QMenu::aboutToShow, this, &QgsSurveyLayerProperties::aboutToShowStyleMenu );

  buttonBox->addButton( mBtnStyle, QDialogButtonBox::ResetRole );

  mBtnMetadata = new QPushButton( tr( "Metadata" ), this );
  QMenu *menuMetadata = new QMenu( this );
  mActionLoadMetadata = menuMetadata->addAction( tr( "Load Metadata…" ), this, &QgsSurveyLayerProperties::loadMetadataFromFile );
  mActionSaveMetadataAs = menuMetadata->addAction( tr( "Save Metadata…" ), this, &QgsSurveyLayerProperties::saveMetadataToFile );
  menuMetadata->addSeparator();
  menuMetadata->addAction( tr( "Save as Default" ), this, &QgsSurveyLayerProperties::saveMetadataAsDefault );
  menuMetadata->addAction( tr( "Restore Default" ), this, &QgsSurveyLayerProperties::loadDefaultMetadata );
...
void QgsSurveyLayerProperties::apply()
{
...
void QgsSurveyLayerProperties::rollback()
{
...
void QgsSurveyLayerProperties::syncToLayer()
{
  ...
  connect( mInformationTextBrowser, &QTextBrowser::anchorClicked, this, &QgsSurveyLayerProperties::openUrl );
...
void QgsSurveyLayerProperties::aboutToShowStyleMenu()
{
...
void QgsSurveyLayerProperties::showHelp()
{
...
void QgsSurveyLayerProperties::crsChanged( const QgsCoordinateReferenceSystem &crs )
{
